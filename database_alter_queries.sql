...
-- 10. UPDATE ALLOCATION TABLE (Add Branch Field and Times)
DO $$
BEGIN
    ...
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'employee_project_allocation' AND column_name = 'start_time') THEN
        ALTER TABLE employee_project_allocation ADD COLUMN start_time TIME;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'employee_project_allocation' AND column_name = 'end_time') THEN
        ALTER TABLE employee_project_allocation ADD COLUMN end_time TIME;
    END IF;
END $$;
...
-- ===================================================================
-- 32. CHANGE LEAVE_MASTER COLUMNS TO DECIMAL FOR HALF-DAY SUPPORT
DO $$
BEGIN
    ALTER TABLE leave_master ALTER COLUMN allotted_days TYPE DECIMAL(10, 2);
    ALTER TABLE leave_master ALTER COLUMN used_days TYPE DECIMAL(10, 2);
    ALTER TABLE leave_master ALTER COLUMN balance_days TYPE DECIMAL(10, 2);
END $$;

-- 33. ADD RADIUS_METERS TO EMPLOYEE_PROJECT_ALLOCATION
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'employee_project_allocation' AND column_name = 'radius_meters') THEN
        ALTER TABLE employee_project_allocation ADD COLUMN radius_meters INTEGER DEFAULT 400;
    END IF;
END $$;
