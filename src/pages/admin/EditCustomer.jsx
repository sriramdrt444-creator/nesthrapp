...
import { ChevronLeft, MapPin, Building, Crosshair, Upload, AlertCircle, Save } from 'lucide-react';
...
const EditCustomer = () => {
  ...
  const loadCustomer = async () => {
    ...
    try {
      ...
      if (result.success) {
        const customer = result.customers.find(c => String(c.customer_id) === String(id));
        if (customer) {
          ...
          setRadius(customer.radius_meters ? String(customer.radius_meters) : '100');
          setContactPerson(customer.customer_contact_person || '');
          setContactEmail(customer.customer_contact_email || '');
          setContactPhone(customer.customer_contact_phone || '');
          setGstNumber(customer.gst_number || '');
          setIndustryType(customer.industry_type || '');
          setLogoPreview(customer.customer_logo || '');
          setStatus(customer.status || 'Active');
          setCustomerBranch(customer.customer_branch || '');
        } else {
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      const result = await updateCompanyCustomer(adminData.company_id, id, customerData);
      if (result.success) {
  ...
  if (loading) {
    return <div style={{ padding: '20px', textAlign: 'center' }}>Loading customer details...</div>;
  }
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column', backgroundColor: 'var(--light-bg)' }}>
      ...
      <div style={{ backgroundColor: 'var(--card-white)', padding: '20px', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', borderBottom: '1px solid #E2E8F0' }}>
        ...
        />
        <h1 style={{ fontSize: '16px', fontWeight: 'var(--font-semibold)', margin: 0 }}>Edit Customer</h1>
      </div>
        ...
        <form onSubmit={handleSave} className="card" style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          <div style={{ borderBottom: '1px solid #E2E8F0', paddingBottom: '16px' }}>
            ...
            <div className="input-group" style={{ marginBottom: 0 }}>
              ...
              <div style={{ border: '2px dashed #CBD5E1', borderRadius: '8px', padding: '16px', textAlign: 'center', cursor: 'pointer' }}>
                ...
                <label htmlFor="logo-upload" style={{ cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px' }}>
                  ...
                  {logoPreview ? <img src={logoPreview} alt="Logo" style={{ width: '60px', height: '60px', objectFit: 'contain' }} /> : <span>Click to upload</span>}
                </label>
              </div>
            </div>
            ...
            <div className="input-group" style={{ marginBottom: 0 }}>
              ...
              <select className="input-field" style={{ paddingLeft: '16px' }} value={customerBranch} onChange={(e) => setCustomerBranch(e.target.value)}>
            ...
            <div className="input-group" style={{ marginBottom: 0 }}>
              ...
              <select className="input-field" style={{ paddingLeft: '16px' }} value={status} onChange={(e) => setStatus(e.target.value)}>
